...
echo "run_geth_in_app.sh started at $(date)" > "$INIT_LOG"
echo "HOME: $HOME" >> "$INIT_LOG"
echo "User: $(whoami)" >> "$INIT_LOG"
echo "Script path: ${BASH_SOURCE[0]}" >> "$INIT_LOG"
SCRIPT_DIR_DEBUG=$(cd "$(dirname "${BASH_SOURCE[0]}")" &>/dev/null && pwd)
echo "SCRIPT_DIR_DEBUG: $SCRIPT_DIR_DEBUG" >> "$INIT_LOG"
echo "Attempting to write to main log next..." >> "$INIT_LOG"

# Main log file
LOG_FILE="$HOME/.marscredit/logs/geth.log"
...
mkdir -p "$DATA_DIR" "$KEYSTORE_DIR" "$DATA_DIR/logs" # Ensure logs dir exists for sure
...
echo "run_geth_in_app.sh: Geth binary found and is executable. Launching..." >> "$LOG_FILE"
...
GETH_PID=$!
echo "run_geth_in_app.sh: Geth launched with PID $GETH_PID. Outputting to $LOG_FILE" >> "$LOG_FILE"
echo $GETH_PID > "$DATA_DIR/geth.pid"
echo "run_geth_in_app.sh: PID $GETH_PID saved to $DATA_DIR/geth.pid" >> "$LOG_FILE"

# Wait for geth to start and check its status
echo "run_geth_in_app.sh: Sleeping for 5s before pgrep check" >> "$LOG_FILE"
sleep 5
if ! pgrep -F "$DATA_DIR/geth.pid" > /dev/null; then
    echo "run_geth_in_app.sh: ERROR - Geth process not found via pgrep -F after 5s. Checking $LOG_FILE for Geth errors." >> "$LOG_FILE"
    # exit 1 # Keep the script running to see curl checks
fi
echo "run_geth_in_app.sh: pgrep check done (or skipped if Geth not found)." >> "$LOG_FILE"

# Wait for RPC endpoint to become available
echo "run_geth_in_app.sh: Starting RPC check loop." >> "$LOG_FILE"
for i in {1..30}; do
    echo "run_geth_in_app.sh: RPC check attempt $i/30..." >> "$LOG_FILE"
...
echo "run_geth_in_app.sh: RPC check loop finished." >> "$LOG_FILE"
